...
#define TEMPO_MIN_FORNECEDOR 1000
#define TEMPO_MAX_FORNECEDOR 2000
...
void* fornecedor_pao(void* arg) {
    while (qtd_clientes>0) {
        ...
        usleep(rand() % TEMPO_MIN_FORNECEDOR + TEMPO_MAX_FORNECEDOR); // Tempo de entrega aleatório entre 1s e 3s
    }
    return NULL;
}
...
int main() {
    ...
    double tempo_medio_espera = tempo_espera_total / total_clientes;
